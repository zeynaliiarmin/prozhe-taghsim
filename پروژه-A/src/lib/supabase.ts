...
export const submissionToDbRow = (submission: Submission): Record<string, unknown> => {
  const { id, created_at, updated_at, fullPhone, full_phone, ...payload } = submission || {};
  return {
    ...(id != null ? { id } : {}),
    full_phone: fullPhone ?? full_phone ?? (payload as Record<string, unknown>)?.fullPhone ?? null,
    payload: {
      ...payload,
...
export const dbRowToSettings = (row: Record<string, unknown> | null): AppSettings | null => {
  if (!row) return null;
  if (row.settings && typeof row.settings === 'object') return row.settings as AppSettings;
  if (row.payload && typeof row.payload === 'object') return row.payload as AppSettings;
  return row as AppSettings;
};

export const settingsToDbRow = (settings: AppSettings): Record<string, unknown> => ({
  key: SETTINGS_KEY,
  settings,
  updated_at: new Date().toISOString(),
});
...
export const deleteMultipleSubmissions = async (ids: (string | number)[]): Promise<void> => {
